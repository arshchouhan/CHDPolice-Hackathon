...
exports.logout = async (req, res) => {
  try {
    ...
    // Update user to remove Gmail tokens
    await User.findByIdAndUpdate(userId, {
      gmail_access_token: null,
      gmail_refresh_token: null,
      gmail_token_expiry: null,
...
exports.checkStatus = async (req, res) => {
  try {
    ...
    const isConnected = user.gmail_connected && user.gmail_access_token;
    console.log('Gmail connected:', isConnected);
    
    // If connected, check if token is valid by making a simple API call
    if (isConnected) {
      try {
        // Create a new OAuth client for this request
        const statusClient = new google.auth.OAuth2(
          process.env.GOOGLE_CLIENT_ID,
          process.env.GOOGLE_CLIENT_SECRET,
          getRedirectUri()
        );
        ...
        // Set up auth client with user's tokens
        statusClient.setCredentials({
          access_token: user.gmail_access_token,
          refresh_token: user.gmail_refresh_token
        });
        
        // Create Gmail API client
        const gmail = google.gmail({ version: 'v1', auth: statusClient });
        
        // Make a simple API call to check if token is valid
        const profile = await gmail.users.getProfile({ userId: 'me' });
        console.log('Gmail profile check successful:', profile.data.emailAddress);
        ...
        console.error('Error checking Gmail token validity:', apiError);
